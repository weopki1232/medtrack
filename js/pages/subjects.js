...
function renderSubjectCard(s, loggedMins, expanded) {
  ...
  let html='<div class="card subj-card'+(behind?' subj-behind':'')+'" style="--sc:'+s.color+'"><div style="display:flex;align-items:center;gap:12px;cursor:pointer" onclick="toggleSubjectExpand(\''+s.id+'\')">'+subjIcon(s,'subj-icon')+'<div style="flex:1"><div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap"><span class="subj-name" style="font-size:16px;font-weight:600">'+s.name+'</span>'+priorityBadge(s.priority)+(behind?'<span class="badge" style="background:var(--red,#ef4444)22;color:var(--red,#ef4444);font-size:11px;cursor:pointer;user-select:none" onclick="event.stopPropagation();showBehindSchedulePopup(event,\''+s.id+'\')">⚠&#xfe0e; '+t('subj_behind')+'</span>':'')+'<span class="badge badge-cyan">'+icon2('◷','📅')+' '+(dl>=0?dl+t('subj_d_left'):t('subj_passed'))+'</span>'+(isSubjectInactive(s.id)?'<span class="badge" style="background:var(--bg3);color:var(--muted);font-size:11px">'+icon2('◌','💤')+' '+t('subj_paused')+'</span>':'')+'</div><div style="font-size:12px;color:var(--muted);margin-top:2px">'+fmtMins(loggedMins)+' '+t('subj_logged')+' · '+t('subj_target')+' '+s.targetHours+'h · '+td+'/'+s.topics.length+' '+t('subj_topics_lbl')+'</div></div><div style="text-align:right;min-width:80px"><div class="subj-pct" style="color:'+s.color+'">'+pct+'%</div><div style="font-size:11px;color:var(--muted)">'+t('subj_hours_unit')+'</div></div><span style="font-size:18px;color:var(--muted);margin-left:4px">'+(expanded?'▲':'▼')+'</span></div>';
  ...
  if(expanded){
    ...
    // Topics
    html+='<div><div style="font-size:13px;font-weight:600;margin-bottom:8px">'+t('subj_topics')+'</div><div class="topic-list">'+s.topics.map(function(tp2){const done=Storage.isTopicDone(tp2.id);var rst=topicReviewState(tp2.id);var tmins=getTopicMins(s.id,tp2.name);var revBadge='';var revBtn=rst?'<button class="btn btn-ghost btn-xs" onclick="event.stopPropagation();removeFromReview(\''+tp2.id+'\')" title="'+t('review_remove')+'">'+(rst==='reviewed'?'<span style="color:var(--green)">✓</span>':'<span class="cog-spin">⚙&#xfe0e;</span>')+'</button>':'<button class="btn btn-ghost btn-xs" onclick="event.stopPropagation();sendToReview(\''+tp2.id+'\')" title="'+t('review_send')+'" style="color:var(--muted)">↺</button>';var hasNote=!!topicNotes[tp2.id];return '<div class="topic-row" onclick="toggleTopicUI(\''+tp2.id+'\',\''+s.id+'\')"><div class="topic-check '+(done?'done':'')+'">'+( done?'✓':'')+'</div><span class="topic-name '+(done?'done':'')+'" style="flex:1">'+tTopic(tp2.id,tp2.name)+'</span>'+(tmins>0?'<span style="font-size:11px;color:var(--muted);white-space:nowrap;margin-right:2px">⏱&#xfe0e; '+fmtMins(tmins)+'</span>':'')+revBadge+revBtn+'<button class="btn btn-ghost btn-xs" onclick="event.stopPropagation();openTopicNoteModal(\''+tp2.id+'\',\''+tp2.name.replace(/'/g,"\\'")+'\')" title="'+t('note_title')+'" style="color:'+(hasNote?'var(--primary-l)':'var(--muted)')+'">✎</button><button class="btn btn-ghost btn-xs" onclick="event.stopPropagation();startTimerForTopic(\''+s.id+'\',\''+tp2.name+'\')" title="Study this">▶</button></div>';}).join('')+'</div></div>';
